...
fn split_sections(
    ...
    if take_mapping(value, "mihomo").is_some() {
        log::warn!("keymap: mihomo section ignored (sing-box only build)");
    }
...
"#;
    ...
    // sing-box is the only supported core
    let mut common = value.clone();
    let _ = take_mapping(&mut common, "mihomo");
    let singbox_section = take_mapping(&mut common, "sing-box");

    assert!(
        singbox_section.is_some(),
        "sing-box section should be extracted"
    );

    if let Some(mut core_specific) = singbox_section {
        merge_mappings(&mut common, &mut core_specific);
    }

    // After merge, common should have connections from sing-box
